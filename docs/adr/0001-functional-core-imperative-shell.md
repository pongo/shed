...
Core tests use in-memory models for stale rules, hidden rules, sorting, Archive bucket paths, conflict resolution, merge planning, and summaries. Adapter tests may use temporary directories for Windows-specific filesystem behavior such as hidden attributes, creation time extraction, symlink handling, and rename behavior.
